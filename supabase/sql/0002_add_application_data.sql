-- Adds storage for the wizard's actual form content (steps 2-4: company
-- details, partner type, customer segments, etc). 0001 only tracked
-- status/version/tenantId for the LINE Service handshake -- without this
-- column a reviewer approving/rejecting via the Supabase table editor would
-- have nothing to read. Run after 0001, same way (paste into SQL Editor).
...
-- instead of replacing it -- drop the old signature explicitly first.
